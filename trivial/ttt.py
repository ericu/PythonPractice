...
def draw_data_line(dataLine):
    return (
        "".join(map(lambda d: "| " + show_player(d) + " ", dataLine)) + "|\n"
    )


def draw_board(data):
    lineLen = len(data[0])
    return draw_h_line(lineLen) + "".join(
        map(
            lambda dataLine: draw_data_line(dataLine) + draw_h_line(lineLen),
            data,
        )
    )
        ...
        for col in range(3):
            if board[row][col] == NO_PLAYER:  # it's a legal move
                ...
                if board_full(newBoard):
                    ...
                    (_, _, bestForOpponent) = best_result_and_move(
